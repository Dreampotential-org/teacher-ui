(function() {
    ...
    // *** UI Elements ***
    const   container   = document.querySelector('.quick-read-container'),
            //inputTxt  = container.querySelector('#txt'),
            speedRdrTxt = container.querySelector('.speedreader-txt .speedrdr-txt'),
            ...
            preloadTxt  = container.querySelector('.preload-txt');
    
    // *** Global TxtReader Functions ***
    const txtReader = {

        // Text Variable
        txt: speedRdrTxt.textContent,
        ...
        UIinit: function() {
            ...
            if (txtReader.txt !== null) {
                ...
                txtReader.splitTxt();
            } else if (preloadTxt && preloadTxt.textContent.trim() !== '') {
                txtReader.txt = preloadTxt.textContent;
                txtReader.splitTxt();
            }
            ...
            // Check if voices added
            let checkVoices = setTimeout(function() {
                if (!txtReader.status.checkVoices) {
                    txtReader.addVoices();
                }
                clearTimeout(checkVoices);
            }, 300);
        ...
        arrowFun: function(e) {
            ...
            // WPM - Plus btn
                ...
                // Insert Value
                txtReader.arrowsInsertVal(true, field, 25, 1, 100000);
            ...
            // WPM - Minus btn
                ...
                // Insert Value
                txtReader.arrowsInsertVal(false, field, 25, 1, 100000);
        ...
        getWordTime: function(word) {
            ...
            //delay longer words that are not numbers > 8
			...
			} else if (word.length < (8 * txtReader.status.chunkSize) && word.length >= (4 * txtReader.status.chunkSize) && txtReader.status.enablemicropauseLongWords === "true" && numbers_matched === null) {

                //delay longer words that are not numbers and ( < 8 && >= 4 )
                let randNum = parseInt(Math.random() * (4 - 1) + 1);

                bonus_factor = Math.ceil(word.length / (randNum*txtReader.status.chunkSize)) * txtReader.status.micropauseLongWordsFactor
        ...
        // Add Voices
        addVoices: function() {
            txtReader.status.checkVoices = true;
            let synth    = window.speechSynthesis,
                voices   = synth.getVoices(),
                dropList = selectVoice.querySelector('.drop-list');
            let watch = setTimeout(function() {
                if (voices.length === 0) {
