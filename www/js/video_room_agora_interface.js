...
function initClientAndJoinChannel(channel, uid) {
  ...
  $.ajax({
      ...
      success: function(response){
        ...
        client.init(response.app_id, function () {
          ...
          joinChannel(channel, uid, response.token); // join channel upon successfull init
        }, function (err) {
...
// when your local video stream is published
...
client.on('stream-added', function (evt) {
  ...
  // get stream object and the id of the remote user that joins the meeting
...
// SCREEN SHARING
async function initScreenShare(agoraAppId, channelName) {
  ...
  screenStream.init(function(){
    ...
    screenClient.join(token, channelName, uid ? uid : null, function(uid) { 
...
function addRemoteStreamMiniView(remoteStream){
  ...
  $('#remote-streams').append(
    $('<div/>', {'id': streamId + '_container',  'class': 'remote-stream-container col-sm-3'}).append(
      $('<div/>', {'id': streamId + '_mute', 
                    'class': 'mute-overlay'}).append(
